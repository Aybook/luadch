...
- [#219](https://github.com/luadch-ng/luadch/issues/219) - Phase 8a F-INF-2: per-field integer clamps on the user accessors `user:share()` / `user:files()` / `user:slots()` / `user:hubs()` in `core/hub_user_object.lua`. The ADC parser is deliberately permissive on integer INF fields (it accepts `^%-?%d+$` so DC++ builds that emit `DS-1` can still log in - Phase 7d / #65 / upstream luadch/luadch#241). Without the accessor clamp, a malicious or buggy client claiming `SS=-1` (negative share) or `SS=10^18` (~10 exabytes) would poison hub-statistic aggregates (`cmd_hubinfo`, `cmd_hubstats`, `etc_records`), the ADC PING reply totals (#147 T1.3 / T1.4), and the HTTP API `/v1/users` / `/v1/stats` JSON serialisation. The strongest single argument for this fix: `etc_records` persists "highest share ever observed" to `scripts/data/etc_records.tbl` via `util.savearray`; pre-PR a single hostile BINF could write `10^18` into the all-time-record file permanently, requiring manual operator intervention to clean. Negative-value handling: negatives are never semantically meaningful in ADC INF integer fields (no such thing as `-1` files / `-1` slots / `-1` hubs); the wire-format permissiveness is purely a compatibility hack. Clamp ranges: `SS=[0, 2^53]` (float-safe ceiling so JSON consumers in JavaScript do not lose precision), `SF=[0, 2^32]` (unsigned-32-bit boundary, generous for real torrenters with many small chunks), `SL=[0, 2^16]`, `HN`/`HR`/`HO=[0, 2^16]`. The Phase 7d parser contract is preserved: the parser still accepts negative/oversize integers and the stored `_inf` is untouched - only Lua consumers reading via the accessors see normalised values. Plugin-author-facing docs updated in `docs/PLUGIN_API.md` user accessor section. New smoke test `test_inf_integer_clamps` doubles as a Phase 7d regression guard (login MUST succeed with the poison BINF). 3.2.x only, not backported.
