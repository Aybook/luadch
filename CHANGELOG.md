...
- [#294](https://github.com/luadch-ng/luadch/issues/294) - HBRI holistic-audit follow-ups (after the #214 / #291 / #286 arc). A final 3-reviewer audit (security / correctness / consistency) found no exploitable or correctness defect; the amplification invariant holds across all three paths. Tier-1 fix ([#295](https://github.com/luadch-ng/luadch/pull/295)): the side-channel port was `tcp_ports[1] or ssl_ports[1]` (plain, falling back to TLS), so a TLS-only family counted as dual-stack and the hub advertised a TLS port in the plain-text `ITCP` - the client opened a plain side-channel to a TLS listener and HBRI silently timed out every attempt (fail-safe, but a misconfiguration trap that contradicted the "plain listener on both families" comments). Now the side-channel port is the first PLAIN port only; a family without a plain listener disables HBRI (secure default). New `tests/unit/hbri_test.lua` locks the `active()` / `eligible()` gate (incl. "no plain port -> inactive"), wired into both CI lua steps. Tier-2 ([#296](https://github.com/luadch-ng/luadch/pull/296)): falsifiable smoke for the wrong-family side-channel reject, the v6-main/v4-secondary direction, post-login disconnect cleanup, and post-login concrete mismatch. 3.2.x only.
