...
- [#222](https://github.com/luadch-ng/luadch/issues/222) (HadesDCH) - post-login INF updates carrying `I4` / `I6` are now silent-stripped from `scripts/hub_inf_manager.lua` instead of killing the user with `ISTA 240` + `TL300`. Real DC++ clients refresh INF (incl. `I4`) on routine triggers (NAT rebind, ISP-IP change, plain refresh); pre-fix those legitimate refreshes triggered FAILED-AUTH disconnects. Anti-spoofing (#97) preserved: stored `_inf` IP is never mutated, broadcast doesn't carry the new claim. Plugin v0.06 -> v0.07. Cherry-picked from master ([PR #224](https://github.com/luadch-ng/luadch/pull/224), commit `7e17b20`).
