...
| GET | `/v1/events` | read | polled event stream; `?since=<id>&types=<csv>` [^http-events-1] |

[^http-events-1]: #263 PR-A. Returns 200 with `data: {events: [...], cursor}`. Each event carries `id` (monotonic per-process counter), `type`, `timestamp` (ISO 8601 UTC), plus per-type payload fields - login (`nick`, `sid`, `level`), logout (`nick`, `sid`), broadcast (`nick`, `sid`, `message`), pm (`from_nick`, `to_nick`, `message`), failed_auth (`nick`, `source_ip`, `reason`), reg_added (`nick`), reg_removed (`nick`), script_error (`message`). Query params: `?since=<id>` (required after the first call; `0` returns from the start of the buffer, `latest` returns empty with the current cursor for no-replay subscription); `?types=<csv>` (optional filter; e.g. `?types=login,logout`). PR-A is **immediate-return** polling only - the response returns whatever has accumulated in the ringbuffer since the supplied cursor. PR-B will add the `?wait=<seconds>` long-poll path via coroutine yield. Ringbuffer is bounded by `cfg.http_events_buffer_size` (default 1000, ~200 KB). If `since` is below the buffer's minimum id (events have been evicted), the response carries `cursor_lost: true` and the client catches up via per-resource endpoints then resumes polling at the returned `cursor`. The `pm` event type is **admin-only** (filtered out for read-scope tokens); other types are read-scope. All string payload fields are control-byte-sanitised at emit time. The 11 event types from the #263 spec - `login`, `logout`, `broadcast`, `pm`, `failed_auth`, `reg_added`, `reg_removed`, `script_error` are wired in PR-A via the `scripts.firelistener` core-tap; `ban_added`, `ban_removed`, `topic_changed` need explicit `http_events.emit` calls from `cmd_ban` / `cmd_topic` (deferred to PR-B's plugin-integration pass).
